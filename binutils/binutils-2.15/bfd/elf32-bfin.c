...
#if 0
  /* These are set in _bfinfdpic_count_got_plt_entries() or later, and this
     function is only called in _bfinfdpic_resolve_final_relocs_info(), that
...
elf32_bfin_print_private_bfd_data (bfd * abfd, PTR ptr)
{
  ...
  /* xgettext:c-format */
  fprintf (file, _("private flags = 0x%lx:"), flags);
...
static bfd_boolean
elf32_bfin_merge_private_bfd_data (ibfd, obfd)
     bfd *ibfd;
     bfd *obfd;
{
  ...
  if (!elf_flags_init (obfd))			/* First call, no flags set.  */
	...
	old_flags |= new_partial;
      ...
      old_partial = (old_flags & ~ EFF_RV_ALL_FLAGS);
      if (new_partial != old_partial)
	{
	  old_flags |= new_partial;
	  error = TRUE;
