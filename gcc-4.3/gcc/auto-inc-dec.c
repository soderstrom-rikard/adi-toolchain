...
find_loop_inc (void)
{
  struct df_ref *use = df_find_use (mem_insn.insn, mem_insn.reg0);
  basic_block use_bb = BASIC_BLOCK (BLOCK_NUM (mem_insn.insn));
  basic_block inc_bb;
  struct df_link *adef;
  struct df_ref *increment = NULL;
  ...
  /* Now, find the initialization and verify the rest of the necessary
     conditions.  */
  for (adef = DF_REF_CHAIN (use); adef; adef = adef->next)
    {
      struct df_ref *init_def = adef->ref;
