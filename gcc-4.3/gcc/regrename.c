	  ...
	  if (dump_file)
	    {
	      fprintf (dump_file, "Register %s in insn %d",
		       reg_names[reg], INSN_UID (this_head->last->insn));
	      if (this_head->need_caller_save_reg)
	      ...
	      enum scan_actions action, enum op_type type, int earlyclobber)
{
  ...
  if (action == mark_write)
	...
	{
	  struct du_head *head
	    = obstack_alloc (&rename_obstack, sizeof (struct du_head));
  ...
  for (p = &open_chains; *p;)
      ...
      else
	{
	  ...
	  if (action == mark_read || action == mark_access)
		...
		{
		  struct du_chain *this_du;
		  this_du = obstack_alloc (&rename_obstack, sizeof (struct du_chain));
		  this_du->next_use = 0;
		  this_du->loc = loc;
		  this_du->insn = insn;
		  this_du->cl = cl;
