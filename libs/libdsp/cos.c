...
/*____________________________________________________________________________
  ...
  Purpose     : Computes cosf(x) using 32-bit floating-point arithmetic.
  Description : The algorithm used to implement this function is adapted from
                ...
                If x is an IEEE NaN or an IEEE Infinity, then cosf will
                return a NaN.

                If x is a denormalized number or 0.0, then cosf will
                return 1.0

                If x is outside the defined domain (see below) then cosf
                will return 0.0
...
_cosf(FLOAT x)
{
    ...
    FLOAT y;        /* Cody and Waite Variables */
    FLOAT xn;
    FLOAT f, g;
    FLOAT result;
    ...
    const FLOAT domain = COS32_X_MAX;
    ...
    */
    ...
    if (exp == 0)
        return 1.0F;            /* when x is a denorm or 0.0 */
    ...
    */
    ...
    /* Calculate the nearest integer to y/PI */

    n  = TO_LONG(ADD(MPY(y, (FLOAT)INV_PI), 0.5F));
    xn = TO_FLOAT(n);

    /* Calculate xn = xn - 0.5 (which is equivalent to adding PI/2 to x) */
