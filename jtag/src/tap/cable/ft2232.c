...
static void
ft2232_clock_schedule_compact( cable_t *cable, int length, uint8_t byte )
{
  params_t *params = (params_t *)cable->params;
  cx_cmd_root_t *cmd_root = &(params->cmd_root);
  ...
  cx_cmd_push( cmd_root, MPSSE_WRITE_TMS |
               MPSSE_LSB | MPSSE_BITMODE | MPSSE_WRITE_NEG );
  cx_cmd_push( cmd_root, length );
  cx_cmd_push( cmd_root, byte );
}
      ...
      case CABLE_CLOCK_COMPACT:
        {
          ...
          if (cable->todo.data[i].action == CABLE_CLOCK_COMPACT)
            {
        ...
        more_cable_clock:
          ...
          if (n + 1 < cable->todo.num_items
              && cable->todo.data[(i + 1) % cable->todo.max_items].action == CABLE_CLOCK
          ...
          if (length)
            {
              if (n + 1 < cable->todo.num_items || how_much == COMPLETELY)
                ft2232_clock_schedule_compact( cable, length - 1, byte | tdi );
              else
                {
                  cable->todo.data[i].action = CABLE_CLOCK_COMPACT;
