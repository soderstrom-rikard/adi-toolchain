 ...
 */
static bus_t *
bf538f_ezkit_bus_new( chain_t *chain, char *cmd_params[] )
{
	bus_t *bus;
	bus = bf537_stamp_bus_new_1 (chain, cmd_params);
	if (bus)
		bus->driver = &bf538f_ezkit_bus;
	return bus;
}
 ...
 */
static bus_t *
bf526_ezkit_bus_new( chain_t *chain, char *cmd_params[] )
{
	bus_t *bus;
	bus = bf537_stamp_bus_new_1 (chain, cmd_params);
	if (bus)
		bus->driver = &bf526_ezkit_bus;
	return bus;
}
 ...
 */
static void
bf526_ezkit_bus_printinfo( bus_t *bus )
{
	int i;
	...
	for (i = 0; i < CHAIN->parts->len; i++)
		if (PART == CHAIN->parts->parts[i])
			break;
	printf( _("Blackfin BF526 EZ-KIT compatible bus driver via BSR (JTAG part No. %d)\n"), i );
}
