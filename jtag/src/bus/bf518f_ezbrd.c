 ...
 */
static bus_t *
bf518f_ezbrd_bus_new( chain_t *chain, const bus_driver_t *driver, char *cmd_params[] )
{
	bus_t *bus;
	part_t *part;
	char buff[15];
	...
	failed |= generic_bus_attach_sig( part, &(AWE),    "AWE_n"  );

	failed |= generic_bus_attach_sig( part, &(ARE),    "ARE_n"  );

	failed |= generic_bus_attach_sig( part, &(ABE[0]), "ABE_n0" );

	failed |= generic_bus_attach_sig( part, &(ABE[1]), "ABE_n1" );

	failed |= generic_bus_attach_sig( part, &(SRAS),   "SRAS_n" );

	failed |= generic_bus_attach_sig( part, &(SCAS),   "SCAS_n" );

	failed |= generic_bus_attach_sig( part, &(SWE),    "SWE_n"  );

	failed |= generic_bus_attach_sig( part, &(SMS),    "SMS_n"  );
...
static int
bf518f_ezbrd_bus_area( bus_t *bus, uint32_t adr, bus_area_t *area )
{
 ...
 */
static void
bf518f_ezbrd_bus_read_start( bus_t *bus, uint32_t adr )
{
	part_t *p = PART;
	chain_t *chain = CHAIN;
 ...
 */
static uint32_t
bf518f_ezbrd_bus_read_next( bus_t *bus, uint32_t adr )
{
	part_t *p = PART;
	chain_t *chain = CHAIN;
	int i;
 ...
 */
static uint32_t
bf518f_ezbrd_bus_read_end( bus_t *bus )
{
	part_t *p = PART;
	chain_t *chain = CHAIN;
	int i;
 ...
 */
static void
bf518f_ezbrd_bus_write( bus_t *bus, uint32_t adr, uint32_t data )
{
	part_t *p = PART;
	chain_t *chain = CHAIN;
 ...
 */
static void
bf518f_ezbrd_bus_printinfo( bus_t *bus )
{
	int i;
...
const bus_driver_t bf518f_ezbrd_bus = {
	"bf518f_ezbrd",
