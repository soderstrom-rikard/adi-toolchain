...
static void
mark_stored_regs (rtx x, rtx pat ATTRIBUTE_UNUSED, void *data)
{
  int *stored = (int *)data;
  int nregs;
  if (!REG_P (x))
...
optimize_loop_addresses (struct loop *loop, basic_block *body)
{
  ...
  edge ph_edge, *exit_edges;
  unsigned ix, num_exits;
  int has_circptr = 0;
  int has_calls = 0;
  ...
  exit_edges = get_loop_exit_edges (loop, &num_exits);
  ...
  for (ix = 0; ix < loop->num_nodes; ix++)
	   ...
	   insn = NEXT_INSN (insn))
	{
	  ...
	  if (NOTE_P (insn) || LABEL_P (insn) || BARRIER_P (insn))
	    continue;
	  ...
	  if (GET_CODE (pat) == USE || GET_CODE (pat) == CLOBBER
	      || GET_CODE (pat) == ASM_INPUT || asm_noperands (pat) >= 0)
	    type = TYPE_MISC;
	  else
  ...
  for (ix = REG_P0; ix < REG_P6; ix++)
	...
	if (!reg_used_otherwise[REG_I0 + iregno]
	    && !REGNO_REG_SET_P (loop->header->il.rtl->global_live_at_start,
				 REG_I0 + iregno)
	...
	{
	  edge e = exit_edges[j];
	  if (REGNO_REG_SET_P (e->dest->il.rtl->global_live_at_start,
			       REGNO (preg)))
	    {
