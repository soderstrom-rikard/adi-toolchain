...
static enum rtx_code
find_extend_code (struct df *df, rtx insn, rtx reg,
		  enum machine_mode narrow_mode)
{
  struct ref *use;
  struct df_link *adef, *uses;
  enum rtx_code known_code = UNKNOWN;

  for (uses = DF_INSN_USES (df, insn); uses; uses = uses->next)
    {
      struct ref *t = uses->ref;
...
optimize_widening_multiply (void)
{
  ...
  for (insn = get_insns (); insn; insn = NEXT_INSN (insn))
	...
	continue;
      ...
      op0_code = find_extend_code (df, insn, XEXP (src, 0), narrow_mode);
      op1_code = find_extend_code (df, insn, XEXP (src, 1), narrow_mode);
...
struct tree_opt_pass pass_fwprop =
{
  "fwprop1",                            /* name */
  gate_fwprop,				/* gate */
  fwprop,				/* execute */
