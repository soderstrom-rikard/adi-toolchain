...
**
** This function implements multiplication for the double type.
...
___muldf3:
	...
	R4 = R1 << 1;
	R4 = R4 | R0;		// Is X 0.0 or -0.0?
	CC = R4 == 0;
	IF CC JUMP .ret_zero;

	R4 = R3 << 1;
	R4 = R4 | R2;		// Is Y 0.0 or -0.0?
	CC = R4 == 0;
	IF CC JUMP .ret_zero;

	// Extract the two exponents eX and eY.
	R7 = (20<<8)|11 (Z);	// 11 bits, at posn 20 in high half
	R4 = EXTRACT(R1, R7.L) (Z);
	R5 = EXTRACT(R3, R7.L) (Z);
...
.return_inf:
	...
	R0 = 0;
	R1 = 0x7FF;			// an exponent of all-ones
	R1 <<= 21;			// aligned at MSB.
