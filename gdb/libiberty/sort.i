...
void fn_1(n, j, bias, top, count, pointers, work)
     size_t n;
  size_t j;
      digit_t *bias;
      digit_t *top;
  unsigned int count[(255  + 1) ];
     void **pointers;
     void **work;
{
      digit_t *digit;
	...
	++count[*digit];
      ...
      for (countp = count + 1; countp < count + (255  + 1) ; ++countp)
	*countp += countp[-1];
     ...
     void **work;
{
  ...
  for (i = 0; i < sizeof (void *) / sizeof (digit_t); ++i)
	...
	j = i;
      ...
      memset (count, 0, (255  + 1)  * sizeof (unsigned int));
...
fn_1(n, j, bias, top, count, pointers, work);
