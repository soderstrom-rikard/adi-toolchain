...
gdbarch_addr_bit (struct gdbarch *gdbarch)
{
  ...
  if (gdbarch_debug >= 2)
    fprintf_unfiltered (gdb_stdlog, "gdbarch_addr_bit called (return value: 0x%x\n",
             gdbarch->addr_bit);
...
gdbarch_deprecated_fp_regnum (struct gdbarch *gdbarch)
{
  ...
  fprintf(stderr, "after \"gdbarch_deprecated_fp_regnum\" value of gdbarch* : 0x%x\n", (int)gdbarch);
...
#ifdef JYOTIK_DEBUG
  fprintf(stderr, "input addr: %x  input len: %d  output addr: %x  output len : %d\n",
           (int)gdb_addr, gdb_len, (int)(*rem_addr), (int)(*rem_len));
#endif
}
