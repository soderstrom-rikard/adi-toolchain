	     ...
	     struct symtab_and_line sal)
{
  ...
  struct frame_id id = get_frame_id(fi);
  int frame_block_addr = frame_address_in_block(fi);
  stb = ui_out_stream_new (uiout);
  old_chain = make_cleanup_ui_out_stream_delete (stb);
  func = find_pc_function (frame_block_addr);//frame_address_in_block (fi));
  if (func)
    {
  ...
  fprintf_symbol_filtered (stb->stream, funname ? funname : "??(Unknown)", funlang,
			   DMGL_ANSI);
