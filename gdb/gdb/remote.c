...
fromhex (int a)
{
  if (a >= '0' && a <= '9')
    ...
    error ("Reply contains invalid hex digit %d(char val: %c): %s@%d", a,(char)a, __FILE__, __LINE__);
}
...
remote_resume (ptid_t ptid, int step, enum target_signal siggnal)
{
  ...
  /* Translate the signal number. Gdb 5.3 does this at gdbserver side.
     But gdbserver for nisa is not compiled in the gdb directory, and
     function target_signal_to_host is not available. So do it here.
     By Qiao Junqi 20030801 */
  siggnal = (enum target_signal)target_signal_to_host(siggnal);
...
Packet: '%s'\n",
	...
	case 'S':		/* Old style status, just signal only */
	  status->kind = TARGET_WAITKIND_STOPPED;
          ...
          /* Translate the signal number. Gdb 5.3 does this at gdbserver side.
             But gdbserver for nisa is not compiled in the gdb directory, and
             function target_signal_from_host is not available. So do it here.
             By Qiao Junqi 20030801 */
          status->value.sig = (enum target_signal) target_signal_from_host(
  ...
  if (remote_protocol_Z[Z_PACKET_SOFTWARE_BP].support != PACKET_DISABLE)
    {
      ...
      *(p++) = ',';
      ...
      putpkt (buf);
      getpkt (buf, (rs->remote_packet_size), 0);
      //fprintf(stderr, "Jyotik_Debug: Calling packet_ok from line %d in file %s\n", __LINE__, __FILE__);
