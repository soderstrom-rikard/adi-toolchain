...
printf("returned from sigwait with WIFSTOPPED %d WSTOPSIG %d\n", WIFSTOPPED (wstat), WSTOPSIG (wstat));
...
fprintf(stderr, "**** calling ptrace to continue ****\n");fflush(stderr);
...
fprintf (stderr, "fetch reg : %d   reg addr : %d   reg size : %d\n",regno, register_addr(regno),  register_size(regno));
#endif

  regaddr = register_addr (regno);
  if (regaddr == -1)
    return;
  buf = malloc (register_size (regno));
  for (i = 0; i < register_size (regno); i += sizeof (PTRACE_XFER_TYPE))
    {
	long val;
      errno = 0;
#ifdef JYOTIK_DEBUG_1
fprintf(stderr, "calling ptrace with patam first = %d  second = %d third = %d fourth = %d\n",
                 PTRACE_PEEKUSER, inferior_pid, (PTRACE_ARG3_TYPE) regaddr, 0);
#endif
      val = 
	ptrace (PTRACE_PEEKUSER, inferior_pid, (PTRACE_ARG3_TYPE) regaddr, 0);
fprintf(stderr, "ptrace returned %x\n", (int)val);
  ...
  if (regno >= 0)
      ...
      for (i = 0; i < register_size (regno); i += sizeof (PTRACE_XFER_TYPE))
	{
	  ...
	  if (errno != 0)
		...
		{
		  char *err = strerror (errno);
		  char *msg = malloc (strlen (err) + 128);
...
{
#ifdef HAVE_LINUX_REGSETS
    fprintf(stderr, "Have linux regsets\n");
  if (use_regsets_p)
    {
...
linux_write_memory (CORE_ADDR memaddr, const char *myaddr, int len)
{
  ...
  /* Allocate buffer of that many longwords.  */
  register PTRACE_XFER_TYPE *buffer = (PTRACE_XFER_TYPE *) malloc (count * sizeof (PTRACE_XFER_TYPE));
  extern int errno;
