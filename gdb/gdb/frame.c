...
/* get frame saved regs by regnum passing as another parameter */
CORE_ADDR *
get_frame_saved_regs_by_regnum (struct frame_info *fi, int regnum)
{
  return fi->saved_regs[regnum];
}

/* save frame regs by regnum */
void 
frame_saved_regs_by_regnum ( struct frame_info *fi, unsigned long size, int regnum)
{

  fi->saved_regs[regnum] = (CORE_ADDR *)
