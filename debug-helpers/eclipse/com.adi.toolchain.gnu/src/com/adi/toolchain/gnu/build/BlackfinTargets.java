...
public class BlackfinTargets implements IManagedOptionValueHandler {
	...
	private static String getTargetProc(IToolChain toolchain) { return getTargetOption(toolchain, Activator.PLUGIN_ID + ".bfin.options.general.proc"); }
	private static String getTargetSirev(IToolChain toolchain) { return getTargetOption(toolchain, Activator.PLUGIN_ID + ".bfin.options.general.sirev"); }
	...
	public boolean handleValue(IBuildObject configuration, IHoldsOptions holder, IOption option, String extraArgument, int event) {
		...
		for (ITool t : toolchain.getTools()) {
			for (IOption o : t.getOptions()) {
				if (!o.getId().startsWith("com.adi.toolchain.gnu.bfin.") || !o.getId().contains(".options.proc"))
					continue;
				try {
	...
	public boolean isEnumValueAppropriate(IBuildObject configuration, IHoldsOptions holder, IOption option, String extraArgument, String enumValue) {
		if (!option.getId().startsWith("com.adi.toolchain.gnu.bfin.") || !option.getId().contains(".options.general.sirev"))
			return true;
