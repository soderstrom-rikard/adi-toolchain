...
public class MMRViewSample extends ViewPart {
	...
	private void hookContextMenu() {
		...
		menuMgr.addMenuListener(new IMenuListener() {
			public void menuAboutToShow(IMenuManager manager) {
				MMRViewSample.this.fillContextMenu(manager);
			}
		});
				...
				for(int i = 0; i < memoryBlocks.length; i++)
				{
					...
					if (memoryBlocks[i] instanceof IMemoryBlockExtension)
					{
						try {
							...
							if(bytes != null)
							{
								byte[] bbb = new byte[bytes.length];
								long value = 0;
								for(int j = 0; j < bytes.length; j++)
								{
									byte b = bytes[j].getValue();
									value = (value << 8) | b;
									bbb[j] = b;
								}
								text += " : 0x" + Long.toHexString(value);
								value++;
								for(int j = 0; j < bytes.length; j++)
								{
									bbb[j] = ((byte)(value & 0xFF));
									value >>= 8;
								}
